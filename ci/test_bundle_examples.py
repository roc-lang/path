...
LOCAL_PACKAGE = 'path: "../package/main.roc"'
...
def copy_examples_with_bundle_url(examples_dir: Path, bundle_url: str) -> list[Path]:
    ...
    for example in examples:
        ...
        example.write_text(source.replace(LOCAL_PACKAGE, f'path: "{bundle_url}"'))
...
def main() -> None:
    ...
    default_tmp = ROOT / ".roc-path-tmp"
    tmp_parent = Path(os.environ.get("ROC_PATH_TMPDIR", default_tmp))
    tmp_parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="roc-path-bundle-", dir=tmp_parent) as tmp:
        tmp_dir = Path(tmp)
